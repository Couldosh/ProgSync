/********
* ec2c version 0.68
* c main file generated for node : periodic 
...
/* Output procedures **********************/
#ifdef CKCHECK
void periodic_BOT_clock(void* cdata){
   _put_bottom("clock");
}
void periodic_BOT_counter(void* cdata){
   _put_bottom("counter");
}
#endif
/* Output procedures **********************/
void periodic_O_clock(void* cdata, _boolean _V) {
   _put_bool("clock", _V);
}
void periodic_O_counter(void* cdata, _integer _V) {
   _put_int("counter", _V);
}/* Main procedure *************************/
int main(){
   ...
   int s = 0;
   /* Context allocation */
   struct periodic_ctx* ctx = periodic_new_ctx(NULL);
   periodic_reset(ctx);
   /* Main loop */
   ISATTY = isatty(0);
   while(1){
      ...
      fflush(stdout);
      ++s;
      periodic_I_c(ctx, _get_bool("c"));
      periodic_I_n(ctx, _get_int("n"));
      periodic_step(ctx);
