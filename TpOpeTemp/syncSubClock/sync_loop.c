/********
* ec2c version 0.68
* c main file generated for node : sync 
...
/* Output procedures **********************/
#ifdef CKCHECK
void sync_BOT_isSync(void* cdata){
   _put_bottom("isSync");
}
#endif
/* Output procedures **********************/
void sync_O_isSync(void* cdata, _boolean _V) {
   _put_bool("isSync", _V);
}/* Main procedure *************************/
int main(){
   ...
   int s = 0;
   /* Context allocation */
   struct sync_ctx* ctx = sync_new_ctx(NULL);
   sync_reset(ctx);
   /* Main loop */
   ISATTY = isatty(0);
   while(1){
      ...
      fflush(stdout);
      ++s;
      sync_I_a(ctx, _get_bool("a"));
      sync_I_b(ctx, _get_bool("b"));
      sync_step(ctx);
