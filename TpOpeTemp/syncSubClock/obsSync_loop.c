/********
* ec2c version 0.68
* c main file generated for node : obsSync 
...
/* Output procedures **********************/
#ifdef CKCHECK
void obsSync_BOT_isEqual(void* cdata){
   _put_bottom("isEqual");
}
#endif
/* Output procedures **********************/
void obsSync_O_isEqual(void* cdata, _boolean _V) {
   _put_bool("isEqual", _V);
}/* Main procedure *************************/
int main(){
   
   int s = 0;
   /* Context allocation */
   struct obsSync_ctx* ctx = obsSync_new_ctx(NULL);
   obsSync_reset(ctx);
   /* Main loop */
   ISATTY = isatty(0);
   while(1){
      ...
      fflush(stdout);
      ++s;
      obsSync_I_a(ctx, _get_bool("a"));
      obsSync_I_b(ctx, _get_bool("b"));
      obsSync_step(ctx);
