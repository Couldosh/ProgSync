/********
* ec2c version 0.68
* c main file generated for node : exclusive 
...
/* Output procedures **********************/
#ifdef CKCHECK
void exclusive_BOT_exclusive(void* cdata){
   _put_bottom("exclusive");
}
#endif
/* Output procedures **********************/
void exclusive_O_exclusive(void* cdata, _boolean _V) {
...
int main(){
   ...
   int s = 0;
   /* Context allocation */
   struct exclusive_ctx* ctx = exclusive_new_ctx(NULL);
   exclusive_reset(ctx);
   /* Main loop */
   ISATTY = isatty(0);
   while(1){
      ...
      fflush(stdout);
      ++s;
      exclusive_I_a(ctx, _get_bool("a"));
      exclusive_I_b(ctx, _get_bool("b"));
      exclusive_step(ctx);
