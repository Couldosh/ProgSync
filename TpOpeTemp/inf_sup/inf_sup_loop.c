/********
* ec2c version 0.68
* c main file generated for node : inf_sup 
...
/* Output procedures **********************/
#ifdef CKCHECK
void inf_sup_BOT_inf(void* cdata){
   _put_bottom("inf");
}
void inf_sup_BOT_sup(void* cdata){
   _put_bottom("sup");
}
#endif
/* Output procedures **********************/
void inf_sup_O_inf(void* cdata, _boolean _V) {
   _put_bool("inf", _V);
}
void inf_sup_O_sup(void* cdata, _boolean _V) {
   _put_bool("sup", _V);
}/* Main procedure *************************/
int main(){
   ...
   int s = 0;
   /* Context allocation */
   struct inf_sup_ctx* ctx = inf_sup_new_ctx(NULL);
   inf_sup_reset(ctx);
   /* Main loop */
   ISATTY = isatty(0);
   while(1){
      ...
      fflush(stdout);
      ++s;
      inf_sup_I_a(ctx, _get_bool("a"));
      inf_sup_I_b(ctx, _get_bool("b"));
      inf_sup_step(ctx);
