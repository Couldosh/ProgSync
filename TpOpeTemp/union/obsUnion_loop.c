/********
* ec2c version 0.68
* c main file generated for node : obsUnion 
...
/* Output procedures **********************/
#ifdef CKCHECK
void obsUnion_BOT_out(void* cdata){
   _put_bottom("out");
}
#endif
/* Output procedures **********************/
void obsUnion_O_out(void* cdata, _boolean _V) {
   _put_bool("out", _V);
}/* Main procedure *************************/
int main(){
   ...
   int s = 0;
   /* Context allocation */
   struct obsUnion_ctx* ctx = obsUnion_new_ctx(NULL);
   obsUnion_reset(ctx);
   /* Main loop */
   ISATTY = isatty(0);
   while(1){
      ...
      fflush(stdout);
      ++s;
      obsUnion_I_a(ctx, _get_bool("a"));
      obsUnion_I_b(ctx, _get_bool("b"));
      obsUnion_step(ctx);
