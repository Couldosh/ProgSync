/********
* ec2c version 0.68
* c main file generated for node : union 
...
/* Output procedures **********************/
#ifdef CKCHECK
void union_BOT_c(void* cdata){
   _put_bottom("c");
}
#endif
/* Output procedures **********************/
void union_O_c(void* cdata, _boolean _V) {
   _put_bool("c", _V);
}/* Main procedure *************************/
int main(){
   ...
   int s = 0;
   /* Context allocation */
   struct union_ctx* ctx = union_new_ctx(NULL);
   union_reset(ctx);
   /* Main loop */
   ISATTY = isatty(0);
   while(1){
      ...
      fflush(stdout);
      ++s;
      union_I_a(ctx, _get_bool("a"));
      union_I_b(ctx, _get_bool("b"));
      union_step(ctx);
