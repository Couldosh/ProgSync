/********
* ec2c version 0.68
* c main file generated for node : AltInit 
...
/* Output procedures **********************/
#ifdef CKCHECK
void AltInit_BOT_err(void* cdata){
   _put_bottom("err");
}
#endif
/* Output procedures **********************/
void AltInit_O_err(void* cdata, _boolean _V) {
   _put_bool("err", _V);
}/* Main procedure *************************/
int main(){
   ...
   int s = 0;
   /* Context allocation */
   struct AltInit_ctx* ctx = AltInit_new_ctx(NULL);
   AltInit_reset(ctx);
   /* Main loop */
   ISATTY = isatty(0);
   while(1){
      ...
      fflush(stdout);
      ++s;
      AltInit_I_a(ctx, _get_bool("a"));
      AltInit_I_b(ctx, _get_bool("b"));
      AltInit_step(ctx);
