/********
* ec2c version 0.68
* c main file generated for node : Alt 
...
/* Output procedures **********************/
#ifdef CKCHECK
void Alt_BOT_err(void* cdata){
   _put_bottom("err");
}
#endif
/* Output procedures **********************/
void Alt_O_err(void* cdata, _boolean _V) {
   _put_bool("err", _V);
}/* Main procedure *************************/
int main(){
   ...
   int s = 0;
   /* Context allocation */
   struct Alt_ctx* ctx = Alt_new_ctx(NULL);
   Alt_reset(ctx);
   /* Main loop */
   ISATTY = isatty(0);
   while(1){
      ...
      fflush(stdout);
      ++s;
      Alt_I_a(ctx, _get_bool("a"));
      Alt_I_b(ctx, _get_bool("b"));
      Alt_step(ctx);
