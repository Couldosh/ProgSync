/********
* ec2c version 0.68
* c main file generated for node : main 
...
/* Output procedures **********************/
#ifdef CKCHECK
void main_BOT_out(void* cdata){
   _put_bottom("out");
}
#endif
/* Output procedures **********************/
void main_O_out(void* cdata, _boolean _V) {
   _put_bool("out", _V);
}/* Main procedure *************************/
int main(){
   ...
   int s = 0;
   /* Context allocation */
   struct main_ctx* ctx = main_new_ctx(NULL);
   main_reset(ctx);
   /* Main loop */
   ISATTY = isatty(0);
   while(1){
