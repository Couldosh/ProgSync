/********
* ec2c version 0.68
* c main file generated for node : Never 
...
/* Output procedures **********************/
#ifdef CKCHECK
void Never_BOT_out(void* cdata){
   _put_bottom("out");
}
#endif
/* Output procedures **********************/
void Never_O_out(void* cdata, _boolean _V) {
   _put_bool("out", _V);
}/* Main procedure *************************/
int main(){
   ...
   int s = 0;
   /* Context allocation */
   struct Never_ctx* ctx = Never_new_ctx(NULL);
   Never_reset(ctx);
   /* Main loop */
   ISATTY = isatty(0);
   while(1){
