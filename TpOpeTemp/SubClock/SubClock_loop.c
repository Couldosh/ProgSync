/********
* ec2c version 0.68
* c main file generated for node : SubClock 
...
/* Output procedures **********************/
#ifdef CKCHECK
void SubClock_BOT_isSubClock(void* cdata){
   _put_bottom("isSubClock");
}
#endif
/* Output procedures **********************/
void SubClock_O_isSubClock(void* cdata, _boolean _V) {
   _put_bool("isSubClock", _V);
}/* Main procedure *************************/
int main(){
   
   int s = 0;
   /* Context allocation */
   struct SubClock_ctx* ctx = SubClock_new_ctx(NULL);
   SubClock_reset(ctx);
   /* Main loop */
   ISATTY = isatty(0);
   while(1){
      ...
      fflush(stdout);
      ++s;
      SubClock_I_a(ctx, _get_bool("a"));
      SubClock_I_b(ctx, _get_bool("b"));
      SubClock_step(ctx);
