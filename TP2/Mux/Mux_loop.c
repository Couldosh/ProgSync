/********
* ec2c version 0.68
* c main file generated for node : Mux 
...
/* Output procedures **********************/
#ifdef CKCHECK
void Mux_BOT_out(void* cdata){
   _put_bottom("out");
}
#endif
/* Output procedures **********************/
void Mux_O_out(void* cdata, _boolean _V) {
   _put_bool("out", _V);
}/* Main procedure *************************/
int main(){
   ...
   int s = 0;
   /* Context allocation */
   struct Mux_ctx* ctx = Mux_new_ctx(NULL);
   Mux_reset(ctx);
   /* Main loop */
   ISATTY = isatty(0);
   while(1){
      ...
      fflush(stdout);
      ++s;
      Mux_I_a(ctx, _get_bool("a"));
      Mux_I_b(ctx, _get_bool("b"));
      Mux_I_set(ctx, _get_bool("set"));
      Mux_step(ctx);
