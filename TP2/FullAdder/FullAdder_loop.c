/********
* ec2c version 0.68
* c main file generated for node : FullAdder 
...
/* Output procedures **********************/
#ifdef CKCHECK
void FullAdder_BOT_sum(void* cdata){
   _put_bottom("sum");
}
void FullAdder_BOT_carry(void* cdata){
   _put_bottom("carry");
}
#endif
/* Output procedures **********************/
void FullAdder_O_sum(void* cdata, _boolean _V) {
   _put_bool("sum", _V);
}
void FullAdder_O_carry(void* cdata, _boolean _V) {
   _put_bool("carry", _V);
}/* Main procedure *************************/
int main(){
   ...
   int s = 0;
   /* Context allocation */
   struct FullAdder_ctx* ctx = FullAdder_new_ctx(NULL);
   FullAdder_reset(ctx);
   /* Main loop */
   ISATTY = isatty(0);
   while(1){
      ...
      fflush(stdout);
      ++s;
      FullAdder_I_a(ctx, _get_bool("a"));
      FullAdder_I_b(ctx, _get_bool("b"));
      FullAdder_I_c(ctx, _get_bool("c"));
      FullAdder_step(ctx);
