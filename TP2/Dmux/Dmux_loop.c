/********
* ec2c version 0.68
* c main file generated for node : Dmux 
...
/* Output procedures **********************/
#ifdef CKCHECK
void Dmux_BOT_a(void* cdata){
   _put_bottom("a");
}
void Dmux_BOT_b(void* cdata){
   _put_bottom("b");
}
#endif
/* Output procedures **********************/
void Dmux_O_a(void* cdata, _boolean _V) {
   _put_bool("a", _V);
}
void Dmux_O_b(void* cdata, _boolean _V) {
   _put_bool("b", _V);
}/* Main procedure *************************/
int main(){
   ...
   int s = 0;
   /* Context allocation */
   struct Dmux_ctx* ctx = Dmux_new_ctx(NULL);
   Dmux_reset(ctx);
   /* Main loop */
   ISATTY = isatty(0);
   while(1){
      ...
      fflush(stdout);
      ++s;
      Dmux_I_in(ctx, _get_bool("in"));
      Dmux_I_sel(ctx, _get_bool("sel"));
      Dmux_step(ctx);
